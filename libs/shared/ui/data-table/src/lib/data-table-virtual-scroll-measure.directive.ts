...
export class DataTableVirtualScrollMeasureDirective implements AfterViewChecked, OnDestroy {
  ...
  #observedRowElements = new Set<HTMLElement>();
  ...
  #scheduleRenderedRowsMeasurement(): void {
    if (this.#measurementTimer !== undefined) {
      return;
    }
  ...
  #observeRenderedRows(rowElements: HTMLElement[]): void {
  ...
  #measureRow(element: HTMLElement, entry?: ResizeObserverEntry): VirtualRowMeasurement {
    return {
  ...
  #heightForElement(element: HTMLElement, entry?: ResizeObserverEntry): number | null {
